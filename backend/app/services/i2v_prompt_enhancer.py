...
_HUMAN_DOGGY_TRIGGER_RE = re.compile(r"\b(?:d0gg1e|Pl0wView)\b", re.IGNORECASE)
...
_ANIMAL_RETRY_USER_SUFFIX = (
    ...
    "Keep the exact animal species they named. Do NOT use man, male, human, guy, or the "
    'doggy_frontview LoRA. Use lora "none".'
)


def _strip_wrapping(text: str) -> str:
    text = text.strip()
    # Some reasoning-capable Ollama models (e.g. Qwen3.5) leak a <think> block
    # even when told not to — strip it defensively before parsing.
    ...
    if (text.startswith('"') and text.endswith('"')) or (text.startswith("'") and text.endswith("'")):
        text = text[1:-1].strip()
    # Fall back to the outermost {...} in case the model still added a preface/note.
...
def _parse_enhancement(raw: str, subject_type: str = "solo_woman") -> dict[str, str]:
    ...
    try:
        ...
        if isinstance(parsed, dict):
            ...
            lora = normalize_lora_id(str(parsed.get("lora", "none")), subject_type)
            if prompt:
...
class I2VPromptEnhancerService:
    ...
    async def enhance(self, user_prompt: str, subject_type: str = "solo_woman") -> dict[str, str]:
        text = user_prompt.strip()
        if not text:
            raise ServiceError("user_prompt cannot be empty")

        subject_type = subject_type if subject_type in ("solo_woman", "couple", "multi_women") else "solo_woman"
        system_prompt = build_i2v_prompt_enhancement_system_prompt(subject_type)
        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": text},
        ]
        raw = await self._llm.complete(messages, options=PROMPT_OPTIONS)
        result = _parse_enhancement(raw, subject_type)
        ...
        logger.info(
            "I2V prompt enhanced via Ollama: input_len=%d output_len=%d lora=%s subject_type=%s",
            len(text),
            len(result["prompt"]),
            result["lora"],
            subject_type,
